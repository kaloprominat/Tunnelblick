...
CFDateRef CreateDateFieldFromCertificate(SecCertificateRef certificate, CFTypeRef oid) {
    const void *keys[] = { oid };
    ...
    if (dict == NULL)
    {
        printErrorMsg("CreateDateFieldFromCertificate: SecCertificateCopyValues", error);
        goto release_ks;
    }
...
CFArrayRef CreateFieldsFromCertificate(SecCertificateRef certificate, CFTypeRef oid) {
    ...
    if (dict == NULL)
    {
        printErrorMsg("CreateFieldsFromCertificate: SecCertificateCopyValues", error);
        CFRelease(keySelection);
        CFRelease(fields);
...
CertDataRef createCertDataFromCertificate(SecCertificateRef certificate) {
    ...
    pCertData->subject = CreateFieldsFromCertificate(certificate, kSecOIDX509V1SubjectName);
    pCertData->issuer = CreateFieldsFromCertificate(certificate, kSecOIDX509V1IssuerName);
...
bool certExpired(SecCertificateRef certificate) {
    bool result;
    CFDateRef notAfter = CreateDateFieldFromCertificate(certificate, kSecOIDX509V1ValidityNotAfter);
    CFDateRef notBefore = CreateDateFieldFromCertificate(certificate, kSecOIDX509V1ValidityNotBefore);
    CFDateRef now = CFDateCreate(kCFAllocatorDefault, CFAbsoluteTimeGetCurrent());
    
    if (!notAfter || !notBefore || !now)
    {
        warnx("CreateDateFieldFromCertificate() returned NULL");
        result = true;
    }
    else
    {
...
SecIdentityRef findIdentity(CertDataRef pCertDataTemplate) {
    ...
    for (int i = 0; i<CFArrayGetCount(result); i++)
    {
        ...
        {
            CFDateRef notBeforeDate = CreateDateFieldFromCertificate(certificate, kSecOIDX509V1ValidityNotBefore);
            if (!notBeforeDate)
            {
                warnx("CreateDateFieldFromCertificate() returned NULL");
                goto release_cert;
